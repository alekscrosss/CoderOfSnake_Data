...
from src.services.auth import get_current_user  # Обновленный импорт

router = APIRouter()

@router.post("/plates/", response_model=schemas_user.Plate)
def add_plate(plate: schemas_user.PlateCreate, db: Session = Depends(get_db), current_user: schemas_user.User = Depends(get_current_user)):
    return crud.crud_plates.add_plate(db, plate)

@router.delete("/plates/{plate_id}", response_model=schemas_user.Plate)
def delete_plate(plate_id: int, db: Session = Depends(get_db), current_admin: schemas_user.User = Depends(get_current_user)):
...
def add_to_blacklist(plate_id: int, db: Session = Depends(get_db), current_admin: schemas_user.User = Depends(get_current_user)):
...
def remove_from_blacklist(plate_id: int, db: Session = Depends(get_db), current_admin: schemas_user.User = Depends(get_current_user)):
